...
class SubprojectDependencies {
    ...
    static void configure(Project p, String commonPath) {
        ...
        boolean automaticArchApi = GeneralUtil.configExt(p)?.automaticArchApi ?: false
        ...
        p.dependencies { DependencyHandler deps ->
            ...
            if (GeneralUtil.isArchEnabled(p)) {
                ...
                if (GeneralUtil.isCommon(p)) {
                    if (automaticArchApi && arch_api_version != null) {
            ...
            if (GeneralUtil.isArchEnabled(p) && GeneralUtil.isMcVersion(p, "1.20.1")) {
