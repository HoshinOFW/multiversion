...
- **`OriginNavigation.FlagFilter`** Walk / nearest / hasTrueSrc functions take an optional filter. New `DECLARATION_ONLY` filter (accepts `OVERWRITE`, `MODSIG`, `NEW`) is used by gutter arrows and keybinds; default `ACCEPT_ANY` keeps the popup's "show all versions" behavior. `MemberVersionView.Inherited` / `ClassVersionView.Inherited` no longer carry the raw origin string
