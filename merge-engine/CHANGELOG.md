...
- **New entry: `MergeEngine.baseVersionFileUpdatePatchedSrc(trueSrcDir, editedRel, editedContent, versionIdx, originMapFile, tolerateParseErrors)`** File-level peer of `baseVersionUpdatePatchedSrc`. Refreshes the base version's `_originMap.tsv` for a single edited rel; no patchedSrc Java output because the base's trueSrc is already the compile source. `editedContent = null` means deletion. Tolerates a missing origin-map file on disk (fresh checkout) and writes a partial file. Returns `BaseFileUpdateResult(writtenFiles, failures)` with `originMapFile` always in `writtenFiles` so callers can drive VFS refresh / cache invalidation uniformly
- **`VersionContext.isBase`** New `Boolean = false` flag. Marks a base-version context so `fileUpdatePatchedSrcWithCascade` / `fileDeletionPatchedSrcWithCascade` dispatch it to `baseVersionFileUpdatePatchedSrc` instead of single-file / sibling-group merge. Explicit flag, not inferred from `baseOriginMapFile == null`, because a downstream version can legitimately have no base map when upstream hasn't been built
- **Cascade dispatch for base** Both cascade entries gain an `isBase` branch at the top of the per-ctx loop. `SKIPPED_UPSTREAM_FAILED` containment works unchanged: a base failure seeds `cascadeFailedRels` and every downstream iteration hits the existing skip check
- **Resilient merges** `processVersion` no longer aborts on the first per-file failure. Per-file, per-target, and per-sibling-group exceptions are collected into a returned failure list and the rest of the merge finalises. `_originMap.tsv` and `.routing` sidecar writes are wrapped in their own try/catch so a sidecar I/O failure doesn't lose the merge work. Inherited-origin walk surfaces previously-silent parse failures
- **Per-target validation isolation** `ModifyClassPreMerge.preMerge` continues past per-group validation and per-file `@ModifyClass` placement / orphan / inner-class failures; collects each into a `failures` list and excludes the failed target rels from `routing` and `virtualTargets`. `Result` gains `failures` and `failedTargetRels` fields
- **Engine-owned `_originMap.tsv` write** `processVersion` writes the (possibly partial) origin map atomically as the final step when an `originMapFile` is supplied, completing the engine ownership of all per-version artefact I/O
- **New entry: `MergeEngine.versionUpdatePatchedSrc(... originMapFile, ...)` returning `VersionMergeResult`** The legacy mutable-`OriginMap` overload is `@Deprecated`; it preserves "throw on first failure" semantics for any external caller
- **New entries: `MergeEngine.fileUpdatePatchedSrcWithCascade` and `fileDeletionPatchedSrcWithCascade`** Take a `List<VersionContext>` (per-version paths + routing + base-origin-map file) plus the edited rel + content + source dir, and orchestrate cross-version propagation themselves. Per-rel containment via in-memory `cascadeFailedRels`: once a rel fails in any version, downstream versions skip the same rel with a `SKIPPED_UPSTREAM_FAILED` failure. Edited content is substituted wherever the engine would read `editedRel` from a directory equal to `editedSourceDir` (handles the "base version mid-save, downstream's base is the editing trueSrc dir" case)
- **New types** `MergeFailure(rel, versionIdx, phase, cause)`, `MergeFailure.Phase` enum (`PRE_MERGE`, `PLAIN_FILE`, `VIRTUAL_TARGET`, `INHERITED_ORIGIN`, `SIDECAR_IO`, `SKIPPED_UPSTREAM_FAILED`), `VersionMergeResult`, `CascadeMergeResult(writtenFiles, failures)`, `VersionContext`
